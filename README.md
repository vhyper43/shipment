[index.html](https://github.com/user-attachments/files/24375303/index.html)
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
</html>[server.js](https://github.com/user-attachments/files/24375320/server.js)

// server.js - Backend for Shipping Tracker
...
// Database setup
const db = new sqlite3.Database('./shipping.db', (err) => {
    if (err) {
...
app.get('/api/shipments/:trackingId', (req, res) => {
    ...
    db.get('SELECT * FROM shipments WHERE tracking_id = ?', [trackingId], (err, shipment) => {
        if (err) {
            res.status(500).json({ error: err.message });
            return;
        }
        ...
        db.all(
            'SELECT * FROM tracking_history WHERE tracking_id = ? ORDER BY created_at DESC',
            [trackingId],
            (err, history) => {
                if (err) {
                    res.status(500).json({ error: err.message });
...
app.post('/api/shipments', (req, res) => {
    ...
    db.run(
        `INSERT INTO shipments (tracking_id, status, origin, destination, weight, estimated_delivery) 
         VALUES (?, ?, ?, ?, ?, ?)`,
        [trackingId, status, origin, destination, parseFloat(weight), estimatedDelivery.toISOString()],
        function(err) {
            if (err) {
                res.status(500).json({ error: err.message });
                return;
            ...
            db.run(
                `INSERT INTO tracking_history (tracking_id, status, location, description) 
                 VALUES (?, ?, ?, ?)`,
                [trackingId, status, origin, 'Shipping label created'],
                function(err) {
                    if (err) {
                        res.status(500).json({ error: err.message });
                        return;
...
app.put('/api/shipments/:trackingId/status', (req, res) => {
    const trackingId = req.params.trackingId;
    const { status, location, description } = req.body;
    const updatedAt = new Date().toISOString();
    
    db.run(
        'UPDATE shipments SET status = ?, updated_at = ? WHERE tracking_id = ?',
        [status, updatedAt, trackingId],
        function(err) {
            if (err) {
                res.status(500).json({ error: err.message });
                return;
            ...
            const statusDescription = description || `Status updated to ${status}`;
            db.run(
                `INSERT INTO tracking_history (tracking_id, status, location, description) 
                 VALUES (?, ?, ?, ?)`,
                [trackingId, status, location, statusDescription],
                function(err) {
                    if (err) {
                        res.status(500).json({ error: err.message });
                        return;
...
const ADMIN_CREDENTIALS = {
    ...
    password: 'uwgu-daniel'  // Change this!
};
...
app.get('/api/seed', (req, res) => {
    ...
    demoData.forEach(shipment => {
        db.run(
            `INSERT OR IGNORE INTO shipments (tracking_id, status, origin, destination, weight, estimated_delivery) 
             VALUES (?, ?, ?, ?, ?, ?)`,
            [shipment.tracking_id, shipment.status, shipment.origin, shipment.destination, shipment.weight, shipment.estimated_delivery],
            function(err) {
                if (err) {
                    ...
                    db.run(
                        `INSERT INTO tracking_history (tracking_id, status, location, description) 
                         VALUES (?, ?, ?, ?)`,
                        [shipment.tracking_id, shipment.status, shipment.origin, 'Demo shipment created']
                    );
                    inserted++;
                }
                ...
                // When all shipments have been processed, send response
                if (processed === demoData.length) {
                    res.json({ 
                        message: `Added ${inserted} demo shipments`,
                        inserted: inserted,
...
app.listen(PORT, () => {
    ...
    console.log(`API available at http://localhost:3000/api/shipments`);
    console.log(`Seed endpoint: http://localhost:3000/api/seed`);
});
// Add to server.js
...
// On hosting platform, set these as environment variables
